...
void process_active_ECU( void* pIn, void* pOut )
{
	...
	if( pstParam )	//如果下个节点不为空，则判断是否为安全算法开关配置
	{
		...
		if( ( pstParam->pcData[0] == 't' ) && ( pstParam->pcData[1] == 'r' )
		        && ( pstParam->pcData[2] == 'u' ) && ( pstParam->pcData[3] == 'e' ) )
		{
			SecurityStatus = true;	//是否需要发送安全算法开关

			//获取安全算法命令偏移
			g_iActiveECURequestSeedOffset = iActiveCmdSum - 2;
			g_iActiveECUSendKeyOffset = iActiveCmdSum - 1;
	...
	if( iActiveCmdSum != 0 )
	{
		...
		if( 1 == *( byte * )pOut ) //如果激活成功
		{
			...
			{
				//提示设置空闲链接失败
				special_return_status( PROCESS_FAIL | NO_JUMP | NO_TIP, NULL, NULL, NULL, 0, 0, NULL, pOut );
			}

			if( SecurityStatus )
			{
				//发送安全进入
				if( !process_security_access_algorithm( 1, pOut ) )
				{
					//提示安全进入失败
					special_return_status( PROCESS_FAIL | NO_JUMP | NO_TIP, NULL, NULL, NULL, 0, 0, NULL, pOut );
				}
